...
Generate formatted Excel workbooks for reconciliation results.
"""
...
class ExcelGenerator(LoggerMixin):
    ...
    def _create_different_sheet(self, wb: Workbook, results: Dict[str, Any]) -> None:
        ...
        self._apply_sheet_formatting(ws, display_df)
        self._highlight_differences(ws, different_df, display_df, results)
    ...
    def _write_dataframe_to_sheet(self, ws, df: pd.DataFrame, freeze_headers: bool = True,
                                add_filters: bool = True, auto_size: bool = False) -> None:
        """
    ...
    def _apply_sheet_formatting(self, ws, df: pd.DataFrame) -> None:
        ...
        # Auto-size columns once after all styling is applied.
        self._auto_size_columns(ws, df)

    def _highlight_differences(self, ws, different_df: pd.DataFrame,
                             display_df: pd.DataFrame, results: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            for diff_row_idx in range(len(different_df)):
                ...
                for field_name, comparison_result in field_comparison.items():
                    ...
                    if is_mismatch:
                        # Find column indices
                        source_col_idx = column_positions[source_col_name]
                        target_col_idx = column_positions[target_col_name]
    ...
    def _auto_size_columns(self, ws, df: pd.DataFrame, min_width: int = 8, max_width: int = 50) -> None:
        ...
        for col_num in range(1, len(df.columns) + 1):
            ...
            max_data_width = 0
            if len(df) > 0:
                column_lengths = df.iloc[:, col_num - 1].astype("string").str.len()
                max_length = column_lengths.max(skipna=True)
                if pd.notna(max_length):
                    max_data_width = int(max_length)
    ...
    def _get_sheet_records(self, ws, results: Dict[str, Any]) -> pd.DataFrame | None:
        """Return the backing dataframe for sheets that represent merged records."""
        if ws.title == 'Matched':
            return results['records']['matched']
        if ws.title == 'Different':
            return results['records']['different']
        return None

    def _build_row_index_lookup(self, original_df: pd.DataFrame, dataset_type: str) -> Dict[Any, List[int]]:
        """Map stable source/target row ids to worksheet row numbers."""
        index_col_name = f'{dataset_type}___index__'
        if index_col_name not in original_df.columns:
            return {}

        row_lookup: Dict[Any, List[int]] = {}
        for excel_row, row_index in enumerate(original_df[index_col_name], start=2):
            if pd.isna(row_index):
                continue
            row_lookup.setdefault(row_index, []).append(excel_row)
        return row_lookup

    def _build_transformation_comment(self, transformation: Dict[str, Any]) -> str:
        """Render a transformation audit record as cell-comment text."""
        ...
        if transformation['transformation_type'] == 'mapping':
            ...
            comment_lines.append(f"Before: {transformation['original_value']}")
            comment_lines.append(f"After: {transformation['new_value']}")
    ...
    def _set_cell_comment(self, cell, comment_text: str, width: int, height: int) -> None:
        """Attach or append a comment, then mark the cell as adjusted."""
        if cell.value is None:
            return

        if cell.comment:
            if comment_text in cell.comment.text:
                return
            cell.comment.text += f"\n\n{comment_text}"
        else:
    ...
    def _add_transformation_comments(self, ws, display_df: pd.DataFrame, results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            original_df = self._get_sheet_records(ws, results)
            ...
            for dataset_type in ['source', 'target']:
                ...
                row_lookup = self._build_row_index_lookup(original_df, dataset_type)
                if not row_lookup:
                    continue
                ...
                for field_name, field_transformations in transformations[dataset_type].items():
                    ...
                    for transformation in field_transformations:
                        row_index = transformation.get('row_index')
                        if row_index not in row_lookup:
                            continue

                        comment_text = self._build_transformation_comment(transformation)
                        for excel_row in row_lookup[row_index]:
                            cell = ws.cell(row=excel_row, column=col_idx)
                            self._set_cell_comment(cell, comment_text, width=300, height=150)
        ...
        try:
            ...
            for key, adj_list in adjustments.items():
                ...
                for adj in adj_list:
                    ...
                    row_index = adj.get('row_index')
                    if row_index not in row_lookup:
                        continue

                    for excel_row in row_lookup[row_index]:
                        cell = ws.cell(row=excel_row, column=col_idx)
                        self._set_cell_comment(cell, comment_text, width=350, height=150)
