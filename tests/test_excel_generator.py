...
class TestExcelGenerator:
    ...
    def test_add_transformation_comments_uses_row_indexes_when_available(self):
        """Transformation comments should attach to the exact transformed row."""
        generator = ExcelGenerator()
        ...
        generator._write_dataframe_to_sheet(ws, display_df)
        ...
        results = {
            ...
            'records': {
                'matched': pd.DataFrame({
                    'source_status': ['A', 'A'],
                    'target_status': ['A', 'A'],
                    'source___index__': [10, 11],
                    'target___index__': [20, 21]
                }),
                'different': pd.DataFrame(),
                'missing_in_source': pd.DataFrame(),
                'missing_in_target': pd.DataFrame()
        ...
        generator._add_transformation_comments(ws, display_df, results)

        assert ws['A2'].comment is None
        assert ws['A3'].comment is not None
        assert "Active -> A" in ws['A3'].comment.text

    def test_add_transformation_comments_skips_without_row_indexes(self):
        """Transformation comments should not guess rows from repeated final values."""
        generator = ExcelGenerator()
    ...
    def test_add_post_merge_comments_uses_row_indexes_when_available(self):
        """Post-merge comments should attach to the exact adjusted row."""
        generator = ExcelGenerator()
        ...
        generator._add_post_merge_comments(ws, display_df, results)

        assert ws['A2'].comment is None
        assert ws['A3'].comment is not None
        assert "Settlement date = reporting date and target MTM is 0" in ws['A3'].comment.text
